...
$exp_active = mysqli_fetch_assoc(mysqli_query($conn, "SELECT is_active FROM experiments WHERE id = 5"))['is_active'];
if (!$exp_active) {
...
.null-state {
  ...
  background: rgba(6,13,26,0.92); backdrop-filter: blur(12px);
  color: #fff; text-align: center; gap: 10px; padding: 32px; z-index: 20;
  animation: fadeIn 0.28s ease;
}
.null-state.show { display: flex; }
.null-icon { font-size: 2.6rem; color: #2ec4e8; opacity: 0.68; margin-bottom: 2px; }
.null-title { font-size: 0.98rem; font-weight: 800; color: rgba(255,255,255,0.95); letter-spacing: -0.01em; }
.null-body { font-size: 0.78rem; color: rgba(255,255,255,0.55); max-width: 270px; line-height: 1.7; }
.null-hint {
  font-size: 0.72rem; margin-top: 2px;
  background: rgba(0,168,212,0.14); border: 1px solid rgba(0,168,212,0.28);
  padding: 6px 16px; border-radius: 20px; color: #7ddcf0;
}
</style>
</head>
<body>
  ...
  <div class="exp-grid">
    ...
    <div class="canvas-panel" style="animation:fadeInUp 0.7s ease 0.2s both">
      <div id="three-mount"></div>
            <!-- NULL STATE OVERLAY -->
      <div class="null-state" id="null-state" role="alert" aria-live="polite">
        <div class="null-icon"><i class="fas fa-route"></i></div>
        <div class="null-title">لا يوجد مسار بصري كلاسيكي</div>
        <div class="null-body">الإعدادات الحالية لا تنتج مساراً بصرياً عبر وجهَي المنشور البصريين. هذا سلوك فيزيائي صحيح تماماً وليس خطأً في البرنامج.</div>
        <div class="null-hint">💡 جرّب زوايا سقوط بين 20°–42° أو غيّر دوران المنشور</div>
      </div>
...
class PrismSim {
  ...
  compute() {
    ...
    for (const w of wls) {
      ...
      if (r) this.results.push({...r, hex:w.hex});
        ...
        const hasRay = this.results.length > 0;
    document.getElementById('null-state').classList.toggle('show', !hasRay);
    this._build3DRays();
    this._buildOverlays();
    this._updatePanel();
    this._updateEmitter();
  ...
  _draw2D() {
    ...
    ctx.globalAlpha=1.0;
        ...
        // Entry/exit dots (تحترم خيار eduPoints)
    if (this.state.eduPoints) {
